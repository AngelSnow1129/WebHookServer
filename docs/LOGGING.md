...
> 该条被标记 `SLOW SQL >= 200ms` 只是首次建表的正常开销,不是性能问题,排障时不要据此告警。该样例采集自历史版本(手写 `CREATE INDEX` 时期),行号指向当时的 `AutoMigrate` 调用点;当前代码同一调用点位于 `main.go:51`。
4. **GORM 日志自带调用点**。`/workspace/SMSServer/main.go:51` 这种绝对路径前缀由 GORM 的 `file:line` 配置产生,会暴露部署路径;应用自身的 `log` 输出则没有调用点(未调用 `log.SetFlags`,默认只有日期时间,不含 `Lshortfile`)。
...
全项目共 **19 处**显式 `log` 调用,分布在 3 个文件。下表为逐点对照。

### `main.go`(11 处)

| 行 | 当前日志 | 建议级别 | 备注 |
|---|---|---|---|
| 28 | `[配置错误] HMAC_SECRET 不能为空` | FATAL | 保留;`log.Fatal` 已具备中止语义 |
| 31 | `[配置错误] WEBHOOK_SECRET 不能为空` | FATAL | 同上 |
| 37 | `[数据库] 连接失败: %v` | FATAL | 保留;建议附加脱敏后的 DSN(host/db,剔除口令) |
| 42 | `[数据库] 获取连接失败: %v` | FATAL | 保留 |
| 52 | `[数据库] 迁移失败: %v` | FATAL | 保留;**已不再出现 `Error 1064`**,见 8.7 |
| 81 | `[服务] 监听地址: %s` | INFO | 保留 |
| 83 | `[服务] 验证码有效期=%v 清理间隔=%v` | INFO | 已补充;解决 TTL/间隔只能从日志倒推的问题 |
| 95 | `[服务] 启动失败: %v` | ERROR | 在 goroutine 中调用 `Fatalf`;理想做法是记录后向主 goroutine 传递错误 |
| 97 | `[服务] 正在关闭... signal=%s` | INFO | 保留;已补充 `signal` 字段 |
| 104 | `[服务] 关闭异常: %v` | ERROR | 保留 |
| 112 | `[服务] 已停止` | INFO | 保留;此行为关闭完成的标志,见 8.6 |
